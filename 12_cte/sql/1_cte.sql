...
SELECT cp.customer_id, c.first_name, c.last_name, total_payment,
FROM customer_payment cp
LEFT JOIN main.customer c ON cp.customer_id = c.customer_id
...
FROM main.film f
INNER JOIN main.film_category fc ON f.film_id = fc.film_id
INNER JOIN main.category c ON c.category_id = fc.category_id)
...
FROM main.category c
INNER JOIN main.film_category fc ON c.category_id = fc.category_id
INNER JOIN main.film f ON fc.film_id = f.film_id
GROUP BY c.name)
